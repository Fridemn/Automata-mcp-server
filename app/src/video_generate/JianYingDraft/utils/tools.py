...
def generate_effect_data(name_or_resource_id: str | int) -> EffectData:
    ...
    return EffectData(guid=generate_id(), resource_id=resource_id, name=name)


def generate_transition_data(
    name_or_resource_id: str | int, duration=0
) -> TransitionData:
    """
    生成转场数据
    :param name_or_resource_id: 动画名称或资源id
    ...
    return TransitionData(
        guid=generate_id(), resource_id=resource_id, duration=duration, name=name
    )


def generate_animation_data(
    name_or_resource_id: str | int,

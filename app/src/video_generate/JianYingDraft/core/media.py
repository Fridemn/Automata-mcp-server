...
class Media:
    ...
    media_category_type_mapping = {"image": "video"}
    ...
    def __init__(self, **kwargs):
        ...
        self.category_type = (
            ""  # 这是媒体添加到草稿里面，素材所属类目（比如图片对应的类目就是Video）
        )
        ...
        media_base_name_no_extension = tools.get_file_name_no_ext(media_file_full_name)
        self.extra_info = (
            media_base_name_no_extension  # media_file_full_name.split("/")[-1]
        )
    ...
    def _set_segment_data_for_content(self):
        ...
        if self.media_type == "text":
            # --- 对于字幕 (text) ---
            # 1. 忽略 "scale"： "scale" 必须在 mediaText.py 中应用到 material.texts["initial_scale"]
            #    (保持 segment["clip"]["scale"] 为模板默认值 1.0)

            # 2. 仅应用 "transform"： "transform" 必须应用到 segment["clip"]["transform"]
            if "transform" in self.kwargs:
                segment["clip"]["transform"] = self.kwargs.get("transform")
        else:
            # --- 对于视频/图片 (video/photo/image) ---
            # (保留原始逻辑)
            # 检查 kwargs 是否有 scale 和 transform
            if "scale" in self.kwargs:
                segment["clip"]["scale"] = self.kwargs.get("scale")
        ...
        }  # 使用原素材的开始位置和使用时长信息（素材自己的时间）
        segment["target_timerange"] = {
            "duration": self.duration / speed,
            "start": 0,
        }  # 放入轨道上的开始位置和使用时长信息（轨道上的时间）

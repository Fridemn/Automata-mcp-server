...
from ..core.mediaEffect import MediaEffect
from ..core.mediaText import MediaText
...
class Draft:
    ...
    def __init__(self, name: str = "", drafts_root: str = ""):
        if not name:
        ...
        if drafts_root:
            ...
            # 默认保存到项目根目录下的 data/jianyin 文件夹
            project_root = os.getcwd()
            self._drafts_root = os.path.join(project_root, "data", "jianyin")
        ...
        self._draft_content_data = tools.read_json(
            os.path.join(template_folder, self._draft_content_file_base_name)
        )
        ...
        self._draft_meta_info_data["tm_draft_create"] = tools.get_timestamp(format=16)
        self._draft_meta_info_data["tm_draft_modified"] = tools.get_timestamp(format=16)
        self._draft_meta_info_data["draft_root_path"] = self._drafts_root.replace(
            "/", "\\"
        )
        self._draft_meta_info_data["draft_removable_storage_device"] = (
            self._drafts_root.split(":/")[0]
        )

        # 为方便调用目标文件中的material部分，定义快捷变量
        self._materials_in_draft_content: {} = self._draft_content_data[
            "materials"
        ]  # 草稿内容库的素材

        self._materials_in_draft_meta_info: {} = self._draft_meta_info_data[
            "draft_materials"
        ]  # 草稿元数据库的素材
        self._videos_material_in_draft_meta_info = self._materials_in_draft_meta_info[
            0
        ]["value"]
        self._audios_material_in_draft_meta_info = self._materials_in_draft_meta_info[
            6
        ][
            "value"
        ]  # type为8的那条

        self._tracks_in_draft_content: [] = self._draft_content_data[
            "tracks"
        ]  # 草稿内容库的轨道
        pass

    def add_media(
        self, media_file_full_name: str, start_at_track=0, duration=0, index=0, **kwargs
    ):
        """
        添加媒体到草稿
        """
    ...
    def add_effect(
        self,
        effect_name_or_resource_id: str | int,
        start=0,
        duration=0,
        index=0,
        **kwargs,
        ...
        media = MediaEffect(
            effect_name_or_resource_id=effect_name_or_resource_id,
            start=start,
            duration=duration,
            **kwargs,
        )
    ...
    def add_subtitle(
        ...
        **kwargs,
        ...
        # 确保 segment_data_for_content 被初始化
        if (
            not hasattr(media, "segment_data_for_content")
            or media.segment_data_for_content is None
        ):
            media._set_segment_data_for_content()

        if position is not None:
            media.set_position(position[0], position[1])

        if font_size is not None:
            media.set_font_size(font_size)
    ...
    def save(self):
        ...
        # 持久化草稿
        draft_content_file_full_name = os.path.join(
            self._draft_folder, self._draft_content_file_base_name
        )
        draft_meta_info_file_full_name = os.path.join(
            self._draft_folder, self._draft_meta_info_file_base_name
        )
    ...
    def __get_track_duration(self, track_type: str):
        ...
        track_duration = (
            last_segment_timerange["start"] + last_segment_timerange["duration"]
        )

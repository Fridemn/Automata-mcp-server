...
from ..utils.dataStruct import TransitionData, AnimationData
...
class MediaVideo(Media):
    ...
    def _set_material_data_for_content(self):
        ...
        self.material_data_for_content["sound_channel_mappings"] = (
            template.get_sound_channel_mapping(scm_id)
        )
        ...
        # 处理转场效果
        transition_data: TransitionData | None = self.kwargs.get(
        ...
        if transition_data:
            ...
            self.material_data_for_content["X.extra_material_refs"].append(
                transition_guid
            )
        pass

        # 处理视频动画
        animation_datas: list[AnimationData] | None = self.kwargs.get(
            "animation_datas", None
        )
        if animation_datas:

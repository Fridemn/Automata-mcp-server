      ...
      <div v-if="cookieStatus" class="p-3 rounded-lg" :class="{
        ...
        <div class="flex flex-col gap-2">
          ...
          <div v-if="cookieStatus.valid && cookieStatus.cookies" class="text-sm text-gray-600">
            <div class="mt-1">
              <span class="font-medium">存储位置:</span> data/cookies/douyin.json
            </div>
            <div class="mt-1">
...
const handleValidateCookies = async () => {
  ...
  try {
    const response = await api.validateDouyinCookies()
    ...
    if (response.data.valid) {
      ...
      const completedStep = {
        ...updatedStep,
        response: {
          message: 'Cookie验证成功，存储在 data/cookies/douyin.json',
          ...response.data
        },
